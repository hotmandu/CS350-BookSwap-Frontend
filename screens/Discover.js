...
import {FlatList, SafeAreaView, View, Text, Image, TextInput, TouchableOpacity, StyleSheet} from 'react-native';
...
export default function Discover({ navigation }) {
    ...
    if (search.length > 0) {
      return (
        ...
        <View style={{padding: 12}}>
          <TextInput style={styles.Searchbar}
            ...
            onSubmitEditing={({nativeEvent})=>{
              ...
              handleSubmit()
              navigation.navigate('Search_page', {screen: 'Discover', params: {keyword: search}})
            }}
            value={search}
            lightTheme="true"
            />
        ...
        <View style={{marginHorizontal:12, height:1, backgroundColor:"rgba(42, 75, 135, 0.25)"}}/>
          <FlatList style={{paddingHorizontal:12}}
            ItemSeparatorComponent={() => <View style={{height:1, backgroundColor: "rgba(42, 75, 135, 0.25)"}}/>}
            data={recent}
            renderItem={({item}) => {
              return (
                ...
                <TouchableOpacity onPress={() => {
                  navigation.navigate('Search_page', {screen: 'Discover', params: {keyword: item}})
                }}>
          ...
          <FlatList style={{padding:12}}
            data={data}
            keyExtractor={({id}) => id}
              renderItem={({item}) => {if ((item.title.toUpperCase().includes(search.toUpperCase()) || (item.author.toUpperCase().includes(search.toUpperCase()))) && search.length > 0) {
                return (
                <Text>
